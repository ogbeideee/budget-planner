...
### FR-28 — Savings plans (concurrent goals with contribution tracking)

- **Standalone records, additive to everything.** A `SavingsPlan` (name, target
  amount, optional target date, optional starting balance) is not a category, a
  budget or a debt: a budget limit is one month's allowance, a debt is money
  owed, a plan is money being set aside toward a target across any number of
  months. Any number of plans may run concurrently; each tracks progress
  independently. Categories with rollover, and debts, behave exactly as they
  did — this feature writes neither.
- **Contributions are ordinary transactions, tagged.** Logging a contribution
  calls the ONE transaction-creation path (`addTransaction`, the same seam
  quick-add uses) with `type: "expense"` and `savingsPlanId` set. There is no
  separate savings write path and no stored balance: saved = starting balance +
  Σ tagged contributions, derived on every read (`lib/savings.ts`, pure and
  deterministic — no React, no store, no I/O; `today` is a parameter).
- **Reports exclusion.** Tagged rows are money moved into a goal, not money
  spent: `totals()`, `spendingByCategory()` and `spendingByCategoryInMonths()`
  skip them on BOTH the expense and income sides. Mirrors why transfers stay
  out of spending — own-goal movement is not consumption.
- **Budget envelopes stay inclusive (deliberate).** `spent()` does NOT skip
  tagged rows, so a contribution drawn against a category consumes that
  category's envelope. This is what makes the rollover integration work: a user
  whose "Savings" category has rollover enabled allocates carried-over funds
  into a plan by contributing against that category — the envelope (base +
  carryover) is drawn down, and at month close only the true remainder carries
  forward. The contribution form shows the chosen rollover category's available
  carryover as a read-only hint; the savings feature never writes a rollover
  record and the month transition is untouched.
- **Pace check (only with a target date).** `currentPerMonth` = Σ contributions
  ÷ elapsed months since creation (a plan younger than a month is judged on one
  month's pace); `requiredPerMonth` = remaining ÷ months to the target date.
  On-track when current ≥ required. A passed deadline with the target unmet is
  behind with everything due now. No target date → no verdict, open-ended
  progress only.
- **Screen:** `/savings`, sidebar **Analytics** section after Debt payoff
  (`16_SAVINGS_PLANS.md`), bottom bar labelled "Save". Ring + bar progress in
  the budget-progress visual language; "on track" / "behind schedule, need ₦X
  more per month" phrasing computed from the plan's own contribution history.
- **Completion is asked, never assumed.** When an active plan reaches its
  target, the screen opens a prompt offering: Mark complete / Keep contributing
  (open-ended, `completedAt` stamped so the prompt never asks again) / Switch to
  ongoing maintenance / Archive. "Decide later" dismisses for the session and
  changes nothing. A reached plan always accepts further contributions.
- **Deletion vs archive.** A plan with no contributions deletes; one with
  contribution history refuses deletion (`in-use-transactions`) and archives
  instead — its rows are ledger history.
- **Schema v10 → v11.** The migration backfills `savingsPlans: []` and NOTHING
  else: it creates no plan and tags no transaction. Contribution tags pointing
  at a plan id that no longer exists are stripped on load rather than failing
  it.
...
interface SavingsPlan {       // FR-28 — standalone goal, not a category/budget/debt
  id: ID;
  ...
  createdAt: string;         // ISO 8601
  completedAt?: string;      // stamped once the reached target is acknowledged
  archivedAt?: string;
}
...
interface Transaction {
  ...
  savingsPlanId?: ID;      // FR-28 — contribution to a savings plan
}
...
interface AppState {
  ...
  savingsPlans: SavingsPlan[]; // FR-28 — concurrent goals; contributions are tagged transactions
  settings: Settings;
}
```
...
- `IncomePlan.expectedAmount` / `IncomePlan.receivedAmount` are non-negative integers; the
  ...
  transactions (FR-18).
- `SavingsPlan.targetDate` absent means open-ended (never an empty string). A transaction's
  `savingsPlanId` references an existing plan or is absent — the validator strips orphan tags
  on load (FR-28).
...
- `saved(plan)` = `startingBalance` + Σ transaction amounts tagged `savingsPlanId`
  (FR-28). Report-level totals exclude tagged rows; budget `spent` includes them.
- `requiredPerMonth(plan)` = `(targetAmount − saved) ÷ months to targetDate`, only when
  `targetDate` is set; pace verdict = on-track when the plan's average monthly
  contribution covers it, else behind (FR-28).
...
| AC-51 | A savings contribution written through `addTransaction` with `savingsPlanId` updates the plan's derived progress, is excluded from Reports expense/income totals and category analysis, still consumes the chosen category's budget envelope (including rollover carryover), and a plan that reaches its target while active is asked — never assumed — what to become: complete, open-ended, ongoing or archived (FR-28) |
